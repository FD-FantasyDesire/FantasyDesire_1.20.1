...
public class AddonSlashUtils extends AttackManager {
    public static EntitySlashEffect doAddonSlash(LivingEntity playerIn, float roll, float YRot, float XRot, int colorCode,float rotationOffset, Vec3 centerOffset, boolean mute, boolean critical, double damage, KnockBacks knockback) {
    ...
    public static EntitySlashEffect doAddonSlashWithEvent(LivingEntity playerIn, float roll, float YRot, float XRot, int colorCode,float rotationOffset, Vec3 centerOffset, boolean mute, boolean critical, double comboRatio, KnockBacks knockback) {
        if (playerIn.level().isClientSide()) {
            return null;
        } else {
            ...
            if (!blade.getCapability(ItemSlashBlade.BLADESTATE).isPresent()) {
                ...
                SlashBladeEvent.DoSlashEvent event = new SlashBladeEvent.DoSlashEvent(blade, (ISlashBladeState)blade.getCapability(ItemSlashBlade.BLADESTATE).orElseThrow(NullPointerException::new), playerIn, roll, critical, comboRatio, knockback);
