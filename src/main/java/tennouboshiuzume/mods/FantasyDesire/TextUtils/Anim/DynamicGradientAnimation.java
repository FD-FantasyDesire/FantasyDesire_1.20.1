...
public class DynamicGradientAnimation implements ITextAnimation {
    private final double speed;

    public DynamicGradientAnimation(double speed) { this.speed = speed; }
    ...
    public MutableComponent apply(String text, StyleData style, long tick) {
        ...
        for (int i = 0; i < len; i++) {
            int rgb = style.colors != null && !style.colors.isEmpty() ?
                    interpolateColor(style.colors, i, len, tick, speed) : 0xFFFFFF;
    ...
    private int interpolateColor(List<Integer> colors, int index, int length, long tick, double speed) {
        if (colors.size() == 1) return colors.get(0);
        double offset = tick * speed / 10.0;
        double f = ((double) index / (length - 1) + offset) % 1.0;
